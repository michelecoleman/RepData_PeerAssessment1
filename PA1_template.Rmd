...
*Note: there is a bug in how data.table's := operator interacts with knitr. Because of that I have suppressed the R output in this chunk only. (The output is thus identical to what it would be if data.table was behaving correctly.) See References section at end for more information.*

```{r results='hide'}
# Add a column with the day of the week
activity_imputed_mean[,dayname := weekdays(as.Date(date))]
...
activity_imputed_mean[dayname %in% c('Saturday','Sunday'), day_type := 'weekend']
...
Here is a discussion of the bug in data.table that causes it to incorrectly display output under knitr when the := operator is used: http://stackoverflow.com/questions/15267018/knitr-gets-tricked-by-data-table-assignment . That discussion notes that this bug has been fixed in data.table version 1.9.5 . That is a development version, however. The latest stable production release on CRAN is still 1.9.4 .
